...
public class LEDManager {
    ...
    public byte[] getConcatenatedArray() {
        synchronized (this) {
    ...
    public Color getLEDColorNormalized(int x, int y, int z) {
        Color color = getLEDColor(x, y, z);
    ...
    public void setLEDColorNormalized(int x, int y, int z, Color color) {
        setLEDColor(x, y, z, new Color(Math.round(color.getRed() / 17F), Math.round(color.getGreen() / 17F), Math.round(color.getBlue() / 17F)));
    }
}
