...
public class JSMinim implements MinimServiceProvider
{
  ...
  public Mixer getOutputMixer()
  {
	...
	public AudioRecordingStream getAudioRecordingStream(String filename, int bufferSize, boolean inMemory) {
            ...
            if (ais != null) {
                ...
                debug("File format class is: " + ais.getFormat().getClass());
                debug("Stream class is: " + ais.getClass());
                ...
                if (format instanceof MpegAudioFormat) {
                ...
                else if (format instanceof FlacAudioFormat) {
                    AudioFormat baseFormat = format;
                    format = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED,
                            baseFormat.getSampleRate(), 16,
                            baseFormat.getChannels(),
                    ...
                    SourceDataLine line = getSourceDataLine(format, bufferSize);
                    if (decAis != null && line != null) {
                        StreamInfo info = getStreamInfo(filename);
                        /*for (Map.Entry<String, Object> prop : baseFormat.properties().entrySet()) {
                            debug("PROP " + prop.getKey() + ": " + prop.getValue());
                        }*/
                        long length = (info.getTotalSamples() * 1000) / info.getSampleRate();
                        debug("LENGTH!!!!!!! " + length);
   ...
   */
	 ...
	 * @throws IOException
	 */
	AudioInputStream getAudioInputStream(InputStream is, String ext)
			throws UnsupportedAudioFileException, IOException
	{
		try
		{
